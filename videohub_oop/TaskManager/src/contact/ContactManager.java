...
public class ContactManager {
    ...
    public void search(String query) {
        for (Contact c : contactArr) {
            if (c != null) {
                if (c.name.toLowerCase().contains(query.toLowerCase()) || c.surname.toLowerCase().contains(query.toLowerCase()) || c.phone.toLowerCase().contains(query.toLowerCase())) {
    ...
    public void deleteContactFromArray(String phone) {
        for (int i = 0; i < contactArr.length; i++) {
            if (contactArr[i] != null && contactArr[i].phone.equals(phone)) {
                contactArr[i] = null;
                break;
            }
        }
